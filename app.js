...
function loadChat(i){messages=chats[i].messages;$("#chat").innerHTML="";messages.forEach(m=>render(m.role,m.content,m.sources||[]))}
function newChat(){
...
async function status(){
  try{const r=await fetch("/api/status");const d=await r.json();$("#status").textContent=d.ok?`● Cloud ready • ${d.model}`:"● Server key missing";$("#status").className=d.ok?"":"bad"}
  catch{$("#status").textContent="● Server offline";$("#status").className="bad"}
}
async function ask(){
  ...
  try{
    const r=await fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messages,personalInstructions:instructions,webSearch:forceWeb||/\b(latest|today|current|news|weather|price|score|schedule|release|version|recent|2026|search|look up|web|update)\b/i.test(text)})});
    const d=await r.json().catch(()=>({}));
    if(!r.ok)throw new Error(d.error||`Server error ${r.status}`);
    typing.remove();messages.push({role:"assistant",content:d.text,sources:d.sources||[]});render("assistant",d.text,d.sources||[]);saveChat();
  }catch(e){typing.remove();render("assistant","⚠️ "+e.message)}
